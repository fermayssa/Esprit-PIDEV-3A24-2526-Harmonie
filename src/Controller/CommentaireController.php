...
final class CommentaireController extends AbstractController
{
    ...
    public function new(Request $request, ForumDomainService $domainService): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                $domainService->saveCommentaire($commentaire);
    ...
    public function edit(Request $request, Commentaire $commentaire, ForumDomainService $domainService): Response
    {
        $form = $this->createForm(CommentaireType::class, $commentaire);
        $form->handleRequest($request);
    ...
    public function delete(Request $request, Commentaire $commentaire, ForumDomainService $domainService): Response
    {
        if ($this->isCsrfTokenValid('delete'.$commentaire->getIdCommentaire(), $request->getPayload()->getString('_token'))) {
            try {
                $domainService->removeCommentaire($commentaire);
            } catch (\DomainException $e) {
                $this->addFlash('danger', $e->getMessage());
            }
        }
