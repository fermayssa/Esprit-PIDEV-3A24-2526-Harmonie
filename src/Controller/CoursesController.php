...
final class CoursesController extends AbstractController
{
    ...
    public function new(Request $request, CourseDomainService $domainService): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                $domainService->saveCourses($course);
    ...
    public function edit(Request $request, Courses $course, CourseDomainService $domainService): Response
    {
        $form = $this->createForm(CoursesType::class, $course);
        $form->handleRequest($request);
    ...
    public function delete(Request $request, Courses $course, CourseDomainService $domainService): Response
    {
        if ($this->isCsrfTokenValid('delete'.$course->getId(), $request->getPayload()->getString('_token'))) {
            try {
                $domainService->removeCourses($course);
            } catch (\DomainException $e) {
                $this->addFlash('danger', $e->getMessage());
            }
        }
